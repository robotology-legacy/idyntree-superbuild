...
")
    ...
    endforeach()
    set(code "cmake_minimum_required(VERSION ${CMAKE_MINIMUM_REQUIRED_VERSION})\n${code}set(command \"${cmd}\")${code_execute_process}")
...
function(ExternalProject_Add_Step name step)
  ...
  if("${command}" STREQUAL "")
    # Some generators (i.e. Xcode) will not generate a file target if no command
    # is set, and therefore the dependencies will be broken.
    # We set a command that does nothing to avoid this issue.
  ...
  get_property(step_targets_set TARGET ${name} PROPERTY _EP_STEP_TARGETS SET)
  if(step_targets_set)
  ...
  get_property(independent_step_targets_set TARGET ${name} PROPERTY _EP_INDEPENDENT_STEP_TARGETS SET)
  if(independent_step_targets_set)
...
function(_ep_add_download_command name)
  ...
  # If source is not in build directory it should be optionally saved (since CMake 3.1.0)
  set(source_dir_persistent 0)
  if(NOT source_in_binary_dir)
    get_property(source_dir_persistent_set TARGET ${name} PROPERTY _EP_SOURCE_DIR_PERSISTENT SET)
    if(source_dir_persistent_set)
      get_property(source_dir_persistent TARGET ${name} PROPERTY _EP_SOURCE_DIR_PERSISTENT)
    else()
      get_property(ep_source_dir_persistent_set DIRECTORY PROPERTY EP_SOURCE_DIR_PERSISTENT SET)
      if(ep_source_dir_persistent_set)
        get_property(source_dir_persistent DIRECTORY PROPERTY EP_SOURCE_DIR_PERSISTENT)
      elseif(NOT CMAKE_MINIMUM_REQUIRED_VERSION VERSION_LESS 3.1.0)
        set(source_dir_persistent 1)
      endif()
    endif()
  endif()
